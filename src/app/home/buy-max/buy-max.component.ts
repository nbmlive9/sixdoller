...
@Component({
  selector: 'app-buy-max',
  templateUrl: './buy-max.component.html',
  styleUrls: ['./buy-max.component.scss']
})
export class BuyMaxComponent {
...
getpackages() {
  this.api.GetPackages().subscribe((res: any) => {
    ...
    this.pack = res.data.filter((pkg: any) => pkg.ptype === 'max');
  });
}
    ...
    if (this.form.valid) {
      ...
      if (this.pdata.topupstatus == '0') {
        ...
        setTimeout(() => {
          ...
          this.router.navigateByUrl('/bitraze-max'); // reload component
        }, 1000);
          },
          (err: any) => {
        ...
        this.api.Topup(val).subscribe(
          ...
          this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
            this.router.navigate(['/bitraze-max']);
          });
        }, 2000);
          },
          (err: any) => {
